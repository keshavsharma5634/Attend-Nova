...
def teacher_screen():
    ...
    /* Disabled buttons styling fix to prevent weird layout drops */
    ...
    div[data-testid="stTextInput"] > div { border-radius: 16px !important; border: 2px solid #e2e8f0 !important; }
    div[data-testid="stTextInput"] > div:has(input:focus) { border: 2px solid #4f46e5 !important; }

    .back-btn-container { display: flex; justify-content: flex-end; width: 100%; }
    </style>
...
def teacher_tab_take_attendance():
    ...
    # ─── ADDED SPINNER FOR SUBJECTS LOADING ───
    ...
    with c1:
        if st.button('Clear all photos', width='stretch', type='tertiary', icon=':material/delete:'):
            if not has_photos:
                st.info("No photos uploaded yet to clear!")
            else:
                st.session_state.attendance_images = []
                st.toast("Photos cleared successfully!")
                st.rerun()
    ...
    with c2:
        if st.button('Run Face Analysis', width='stretch', type='secondary', icon=':material/analytics:'):
            if not has_photos:
                st.error("Please add classroom photos first using 'Add Photos' button!")
            else:
...
def teacher_tab_attendance_records():
    ...
    # ─── SPINNER ALREADY COVERING ENTIRE LOGIC BLOCK HERE ───
...
def teacher_screen_login():
    ...
    with c2:
        ...
        if st.button("Back", key='loginbackbtn'):
    ...
    teacher_username = st.text_input("Enter username", placeholder='keshav123')
